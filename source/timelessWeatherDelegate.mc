...
class timelessWeatherDelegate extends System.ServiceDelegate {
    ...
    const CURRENT_CITY_WEATHER_URI = "https://api.openweathermap.org/data/2.5/weather?q=$1$&appid=8c401201165447badd5d8cbf631492a3&units=$2$";
    const CURRENT_COORD_WEATHER_URI = "https://api.openweathermap.org/data/2.5/weather?lat=$1$&lon=$2$&appid=8c401201165447badd5d8cbf631492a3&units=$3$";
    const FORECAST_CITY_WEATHER_URI = "https://api.openweathermap.org/data/2.5/forecast?q=$1$&appid=8c401201165447badd5d8cbf631492a3&units=$2$&cnt=4";
    const FORECAST_COORD_WEATHER_URI = "https://api.openweathermap.org/data/2.5/forecast?lat=$1$&lon=$2$&appid=8c401201165447badd5d8cbf631492a3&units=$3$&cnt=4";
    ...
    function onTemporalEvent() {
	    ...
	    if (positionInfo != null) {
            ...
            System.println("Refresh location " + lattitude + ", " + longitude);
        }
    ...
    function getCurrentWeatherURI() {
        if (lattitude != null && longitude != null) {
	       return Lang.format(CURRENT_COORD_WEATHER_URI, [lattitude, longitude, getTemperatureUnits()]);
	    } else {
	       return Lang.format(CURRENT_CITY_WEATHER_URI, [Communications.encodeURL(App.getApp().getProperty("WeatherLocation")), getTemperatureUnits()]);
	    }	    
    }
    
    function getForecastWeatherURI() {
	    if (lattitude != null && longitude != null) {
	       return Lang.format(FORECAST_COORD_WEATHER_URI, [lattitude, longitude, getTemperatureUnits()]);
	    } else {
	       return Lang.format(FORECAST_CITY_WEATHER_URI, [Communications.encodeURL(App.getApp().getProperty("WeatherLocation")), getTemperatureUnits()]);
	    }	    
    }
    ...
    function currentWeatherCallback(responseCode, data) {
        ...
        // process.
        System.println("Current response "+ data);
